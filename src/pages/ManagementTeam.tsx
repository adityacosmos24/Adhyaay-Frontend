...
import Coordinator, { teamMembers3D as coordinatorData } from "../components/ui/cordinator"; // <-- fixed
    ...
    <div className="pt-16 flex flex-col items-center gap-10 w-full bg-gray-50">
      ...
      <div className="flex flex-col gap-12 w-full px-4 md:px-16">
        {teamProfiles.map((profile, idx) => (
          ...
          >
            <div className="md:w-1/2 relative group rounded-xl overflow-hidden shadow-lg">
              <img
                src={profile.src}
                alt={profile.name}
                className="w-full h-64 md:h-80 object-cover rounded-xl transform transition-all duration-500 group-hover:scale-105"
              />
              ...
              <div className="absolute bottom-0 left-0 w-full p-3 md:p-4">
                ...
                <p className="text-orange-200 opacity-90 text-sm md:text-base">{profile.role}</p>
              </div>
            </div>

            <div className="md:w-1/2 bg-white rounded-xl shadow-lg p-5 md:p-6 border-l-4 border-orange-500 transform transition-transform duration-500 hover:-translate-y-2 hover:shadow-2xl">
              <p className="text-gray-700 italic text-base md:text-lg">"{profile.quote}"</p>
            </div>
          </div>
        ))}
      </div>
